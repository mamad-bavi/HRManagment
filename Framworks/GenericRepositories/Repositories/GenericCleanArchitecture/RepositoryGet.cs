...
namespace GenericRepositories.Repositories.GenericCleanArchitecture
{
        ...
        where TEntity : class
    {
        ...
        {
            var entityType = DbQueryContext.Model.FindEntityType(typeof(TEntity));

            var key = entityType?.FindPrimaryKey();

            var idProperty = key?.Properties.FirstOrDefault()?.Name;

            var entity = TableNoTracking
                .FirstOrDefault(p =>
                EF.Property<object>(p, idProperty) == (ids[0]));
            ...
            params object[] ids)
        {
            ...
            var idProperty = key?.Properties.FirstOrDefault()?.Name;
            var entity = await TableNoTracking.FirstOrDefaultAsync(c =>
            EF.Property<object>(c, idProperty) == ids[0]);
            ...
            params object[] ids)
        {
            ...
            return entity.ConvertObject<TDtoGetById, TEntity>(mapper);
        }
