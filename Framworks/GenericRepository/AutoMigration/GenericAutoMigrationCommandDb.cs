...
namespace GenericRepository.AutoMigration
{
    ...
    List<MigrationOperation> operations)
        {
            SchemaSnapshot oldSnapshotwithDontChanges = new()
            {
                Tables = oldSnapshot.Tables.Where(c => 
                c.ForeignKeys != null && c.ForeignKeys.Count() > 0)
               .ToList()

            };
            //foreach (var item in oldSnapshot.Tables.Where(c => !newSnapshot.Tables.Any(n => n.Name == c.Name && n.Schema == c.Schema)).ToList())
            //{
            //    //if(!newSnapshot.Tables.Any(c => c.Name == item.Name && c.Schema == item.Schema ))
            //        oldSnapshotwithDontChanges.Tables.Add(item);
            //}
            


            foreach (var oldTable in oldSnapshotwithDontChanges.Tables)
            {

                if (oldTable.Name == "UserRole")
                {
